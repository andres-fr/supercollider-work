...
Find the parameters for the signal \(S'\) (the {\it reconstruction}):
  ...
  \begin{align*}
    \begin{aligned}
      ...
      &= \underset{\vec{k}, \vec{d}}{\text{minimize}} \sum_{t=1}^{|S|}\Bigg\{\bigg(S{\scriptstyle (t)}-\sum_{n=1}^N\{\varphi_n(k_nm_n{\scriptstyle (t+d_n)})\}\bigg)^2\Bigg\}
    \end{aligned}
  \end{align*}
  Whereas \(\vec{k}=(k_1, ..., k_N)\) and \(\vec{d}=(d_1, ..., d_N)\) are respectively the normalization and delay vectors that hold the values for each corresponding material with the same index, and \(\vec{k}^*, \vec{d}^*\) are the values of those vectors that minimize the expressed function. As it will be shown, the solutions have the following values:
  \begin{align*}
    \begin{aligned}
      &\vec{d}_n^* = idx(max(CC[S, m_n])) \quad : \quad \forall\, m_n \in M'\\
      &\vec{k}_n^* = 3
    \end{aligned}
\end{align*}
...
First, it helps to reformulate the energy difference to minimize:
  \begin{align*}
    \begin{aligned}
    ...
    \end{aligned}
  \end{align*}
  Whereas \(<x{\scriptstyle (t)},y{\scriptstyle (t)}> = \sum_{t=1}^{max(|x|,|y|)}\{x{\scriptstyle (t)}y{\scriptstyle (t)}\}\) represents the dot-product of two signals \(x, y\). It would now follow a proof, substituting the energy values with the precise definition of \(S'\) and reformuling everything as a second-order polynomial of \(k\). But at this point is when it is possible to profit from the simplified problem definition, with the following observations:
